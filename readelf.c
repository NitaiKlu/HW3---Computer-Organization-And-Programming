...
#define SECTION_NAME_VALID(X) \
	((X) != NULL && filedata->string_table != NULL && (X)->sh_name < filedata->string_table_length)
		 ...
		 const char *reason)
{
	...
	if (filedata->archive_file_offset > filedata->file_size || offset > filedata->file_size - filedata->archive_file_offset || amt > filedata->file_size - filedata->archive_file_offset - offset)
	{
		if (reason)
...
static unsigned int
print_symbol2(signed int width, const char *symbol, int is_null_type)
{
	bfd_boolean extra_padding = FALSE;
	bfd_boolean do_dots = FALSE;
	signed int num_printed = 0;
...
#define MAX_PRINT_SEC_NAME_LEN 128
	...
	/** // addition://////////////////////////////////////////////////////////
	char *name_with_ela = malloc(sizeof(char) * MAX_PRINT_SEC_NAME_LEN);
	strcpy(name_with_ela, name);
	char k;
	int i = 0;
	while ((k = *name++) != 0)
	{
...
find_section(Filedata *filedata, const char *name)
{
	...
	for (i = 0; i < filedata->file_header.e_shnum; i++)
		if (SECTION_NAME_VALID(filedata->section_headers + i) && streq(SECTION_NAME(filedata->section_headers + i), name))
				...
				if (psym->st_name == 0)
				{
					...
					if (ELF_ST_TYPE(psym->st_info) == STT_SECTION)
					{
						if (psym->st_shndx < filedata->file_header.e_shnum)
							sec_name = SECTION_NAME_PRINT(filedata->section_headers + psym->st_shndx);
...
parse_args(struct dump_data *dumpdata, int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "ACDHILNR:STVWacdeghi:lnp:rstuvw::x:z", options, NULL)) != EOF)
	{
		switch (c)
		{
		case 0:
	...
	if (!do_dynamic && !do_syms && !do_reloc && !do_unwind && !do_sections && !do_segments && !do_header && !do_dump && !do_version && !do_histogram && !do_debugging && !do_arch && !do_notes && !do_section_groups && !do_archive_index && !do_dyn_syms && !do_lto_syms)
	{
		if (do_checks)
		{
			check_all = TRUE;
		...
		case PT_DYNAMIC:
			...
			if (filedata->section_headers != NULL)
			{
			   ...
			   loader,  should exactly match the .dynamic section.  */
				if (do_checks && (filedata->dynamic_addr != segment->p_offset || filedata->dynamic_size != segment->p_filesz))
					warn(_("\
the .dynamic section is not the same as the dynamic segment\n"));
			...
			/* PR binutils/17512: Avoid corrupt dynamic section info in the
			   ...
			   sections).  */
			if (filedata->dynamic_addr > filedata->file_size || (filedata->dynamic_size > filedata->file_size - filedata->dynamic_addr))
			{
		 ...
		 i++, section++)
	{
	   ...
	   possibly fill in some file data as well.  */
		...
		case SHT_DYNSYM:
			...
			filedata->dynamic_symbols = GET_ELF_SYMBOLS(filedata, section, &filedata->num_dynamic_syms);
		...
		case SHT_STRTAB:
			if (streq(name, ".dynstr"))
			{
				...
				filedata->dynamic_strings = (char *)get_data(NULL, filedata, section->sh_offset,
															 1, section->sh_size, _("dynamic strings"));
				filedata->dynamic_strings_length = filedata->dynamic_strings == NULL ? 0 : section->sh_size;
	 ...
	 section.  */
	  if (section->sh_type < SHT_LOOS && section->sh_link != 0)
		...
		if (do_section_details)
			...
			print_symbol2(-17, SECTION_NAME_PRINT(section), section->sh_type == SHT_NULL);
...
process_section_groups(Filedata *filedata)
{
	...
	filedata->section_headers_groups = (struct group **)calloc(filedata->file_header.e_shnum,
															   sizeof(struct group *));
		 ...
		 i++, section++)
	{
		if (section->sh_type == SHT_GROUP)
		{
			...
			if (ELF_ST_TYPE(sym->st_info) == STT_SECTION)
			{
				...
				group_name = SECTION_NAME_PRINT(filedata->section_headers + sym->st_shndx);
				strtab_sec = NULL;
				free(strtab);
				strtab = NULL;
				strtab_size = 0;
	...
	while (unwcount-- > 0)
	{
	   ...
	   sections so the loop above should never fail.  */
		...
		if ((unwsec->sh_flags & SHF_GROUP) != 0)
		{
			...
			if (filedata->section_headers_groups == NULL || filedata->section_headers_groups[i] == NULL)
				i = filedata->file_header.e_shnum;
			else
			{
				...
				for (; g != NULL; g = g->next)
				{
					...
					if (SECTION_NAME_VALID(sec) && streq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info))
						break;
				}
		...
		else if (SECTION_NAME_VALID(unwsec) && strneq(SECTION_NAME(unwsec),
													  ELF_STRING_ia64_unwind_once, len))
		{
			...
			for (i = 0, sec = filedata->section_headers;
				 i < filedata->file_header.e_shnum;
				 ++i, ++sec)
				if (SECTION_NAME_VALID(sec) && strneq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info_once, len2) && streq(SECTION_NAME(sec) + len2, suffix))
					break;
		}
		else
		{
			...
			suffix = "";
			if (SECTION_NAME_VALID(unwsec) && strneq(SECTION_NAME(unwsec), ELF_STRING_ia64_unwind, len))
				suffix = SECTION_NAME(unwsec) + len;
			for (i = 0, sec = filedata->section_headers;
				 i < filedata->file_header.e_shnum;
				 ++i, ++sec)
				if (SECTION_NAME_VALID(sec) && strneq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info, len2) && streq(SECTION_NAME(sec) + len2, suffix))
					break;
		}
...
hppa_process_unwind(Filedata *filedata)
{
	...
	for (i = 0, sec = filedata->section_headers; i < filedata->file_header.e_shnum; ++i, ++sec)
	{
		...
		else if (SECTION_NAME_VALID(sec) && streq(SECTION_NAME(sec), ".PARISC.unwind"))
			unwsec = sec;
	}
	...
	for (i = 0, sec = filedata->section_headers; i < filedata->file_header.e_shnum; ++i, ++sec)
	{
		if (SECTION_NAME_VALID(sec) && streq(SECTION_NAME(sec), ".PARISC.unwind"))
		{
...
get_32bit_dynamic_section(Filedata *filedata)
{
	...
	filedata->dynamic_section = (Elf_Internal_Dyn *)cmalloc(filedata->dynamic_nent, sizeof(*entry));
...
get_num_dynamic_syms(Filedata *filedata)
{
	...
	if (filedata->dynamic_info[DT_HASH])
	{
		...
		if (fseek(filedata->handle,
				  (filedata->archive_file_offset + offset_from_vma(filedata, filedata->dynamic_info[DT_HASH],
																   sizeof nb + sizeof nc)),
				  SEEK_SET))
		{
	...
	if (filedata->dynamic_info_DT_GNU_HASH)
	{
		...
		if (fseek(filedata->handle,
				  (filedata->archive_file_offset + offset_from_vma(filedata,
		...
		if (fseek(filedata->handle,
				  (filedata->archive_file_offset + offset_from_vma(filedata, buckets_vma, 4)),
				  SEEK_SET))
		{
			error(_("Unable to seek to start of dynamic information\n"));
			goto no_gnu_hash;
		...
		filedata->gnubuckets = get_dynamic_data(filedata, filedata->ngnubuckets, 4);
		...
		if (fseek(filedata->handle,
				  (filedata->archive_file_offset + offset_from_vma(filedata,
																   buckets_vma + 4 * (filedata->ngnubuckets + maxchain),
																   4)),
				  SEEK_SET))
		{
			error(_("Unable to seek to start of dynamic information\n"));
		...
		if (fseek(filedata->handle,
				  (filedata->archive_file_offset + offset_from_vma(filedata, (buckets_vma + 4 * filedata->ngnubuckets),
																   4)),
				  SEEK_SET))
		{
			error(_("Unable to seek to start of dynamic information\n"));
		...
		{
			if (fseek(filedata->handle,
					  (filedata->archive_file_offset + offset_from_vma(filedata, (buckets_vma + 4 * (filedata->ngnubuckets + maxchain)), 4)),
					  SEEK_SET))
			{
				error(_("Unable to seek to start of dynamic information\n"));
				goto no_gnu_hash;
		...
		for (hn = 0; hn < filedata->ngnubuckets; ++hn)
			if (filedata->gnubuckets[hn] != 0)
			{
				...
				do
				{
					if (filedata->dynamic_info_DT_MIPS_XHASH)
					{
						if (off < filedata->ngnuchains && filedata->mipsxlat[off] >= num_of_syms)
					...
					si++;
				} while (off < filedata->ngnuchains && (filedata->gnuchains[off++] & 1) == 0);
			}
	...
	if (filedata->dynamic_symbols == NULL || do_histogram)
	{
		...
		for (entry = filedata->dynamic_section;
			...
			if (entry->d_tag == DT_SYMTAB)
				...
				filedata->dynamic_info_DT_GNU_HASH = entry->d_un.d_val;
			else if ((filedata->file_header.e_machine == EM_MIPS || filedata->file_header.e_machine == EM_MIPS_RS3_LE) && entry->d_tag == DT_MIPS_XHASH)
			{
				filedata->dynamic_info_DT_MIPS_XHASH = entry->d_un.d_val;
				filedata->dynamic_info_DT_GNU_HASH = entry->d_un.d_val;
			}
		...
		if (num_of_syms != 0 && filedata->dynamic_symbols == NULL && filedata->dynamic_info[DT_SYMTAB] && filedata->dynamic_info[DT_SYMENT])
		{
			Elf_Internal_Phdr *seg;
				 ...
				 ++seg)
			{
				...
				if (vma >= (seg->p_vaddr & -seg->p_align) && vma < seg->p_vaddr + seg->p_filesz)
				{
					...
					section.sh_size = (num_of_syms * filedata->dynamic_info[DT_SYMENT]);
					section.sh_entsize = filedata->dynamic_info[DT_SYMENT];

					if (do_checks && filedata->dynamic_symtab_section != NULL && ((filedata->dynamic_symtab_section->sh_offset != section.sh_offset) || (filedata->dynamic_symtab_section->sh_size != section.sh_size) || (filedata->dynamic_symtab_section->sh_entsize != section.sh_entsize)))
						warn(_("\
the .dynsym section doesn't match the DT_SYMTAB and DT_SYMENT tags\n"));

					section.sh_name = filedata->string_table_length;
					filedata->dynamic_symbols = GET_ELF_SYMBOLS(filedata, &section,
																&filedata->num_dynamic_syms);
					if (filedata->dynamic_symbols == NULL || filedata->num_dynamic_syms != num_of_syms)
					{
	...
	if (filedata->dynamic_strings == NULL)
			 ...
			 ++entry)
		{
			...
			if (filedata->dynamic_info[DT_STRTAB] && filedata->dynamic_info[DT_STRSZ])
			{
				unsigned long offset;
				...
				offset = offset_from_vma(filedata,
										 ...
										 str_tab_len);
				if (do_checks && filedata->dynamic_strtab_section && ((filedata->dynamic_strtab_section->sh_offset != (file_ptr)offset) || (filedata->dynamic_strtab_section->sh_size != str_tab_len)))
					warn(_("\
the .dynstr section doesn't match the DT_STRTAB and DT_STRSZ tags\n"));

				filedata->dynamic_strings = (char *)get_data(NULL, filedata, offset, 1, str_tab_len,
			 ...
			 ++entry)
		{
			if (entry->d_tag == DT_SYMINENT)
			{
			   ...
			   So do not use an assert, instead generate an error message.  */
				...
				filedata->dynamic_syminfo_offset = offset_from_vma(filedata, entry->d_un.d_val, syminsz);
		}
		 ...
		 entry++)
	{
		...
		case DT_FINI_ARRAY:
			if (do_dynamic)
			{
				if (entry->d_tag == DT_USED && VALID_DYNAMIC_NAME(filedata, entry->d_un.d_val))
				{
		...
		default:
			if ((entry->d_tag >= DT_VERSYM) && (entry->d_tag <= DT_VERNEEDNUM))
				filedata->version_info[DT_VERSIONTAGIDX(entry->d_tag)] = entry->d_un.d_val;
			...
			for (cnt = 0; cnt < total; cnt += 4)
			{
				...
				for (j = 0; (j < 4) && (cnt + j) < total; ++j)
					...
					default:
						...
						if (data[cnt + j] != 0x8001 && filedata->version_info[DT_VERSIONTAGIDX(DT_VERDEF)])
						{
...
get_symbol_index_type(Filedata *filedata, unsigned int type)
{
	...
	default:
		...
		else if (filedata->file_header.e_shnum != 0 && type >= filedata->file_header.e_shnum)
						  ...
						  unsigned short *vna_other)
{
	...
	*sym_info = (vers_data & VERSYM_HIDDEN) != 0 ? symbol_hidden : symbol_public;
	...
	if (psym->st_shndx != SHN_UNDEF && vers_data != 0x8001 && filedata->version_info[DT_VERSIONTAGIDX(DT_VERDEF)])
	{
		Elf_Internal_Verdef ivd;
		Elf_Internal_Verdaux ivda;
		Elf_External_Verdaux evda;
					 ...
					 char *strtab, size_t strtab_size)
{
	...
	if (ELF_ST_BIND(psym->st_info) == STB_LOCAL && section != NULL && si >= section->sh_info
...
process_lto_symbol_tables(Filedata *filedata)
{
	...
	for (i = 0, section = filedata->section_headers;
		 i < filedata->file_header.e_shnum;
		 i++, section++)
		if (SECTION_NAME_VALID(section) && CONST_STRNEQ(SECTION_NAME(section), ".gnu.lto_.symtab."))
...
process_symbol_table(Filedata *filedata)
{
	...
	if ((filedata->dynamic_info[DT_HASH] || filedata->dynamic_info_DT_GNU_HASH) && do_syms && do_using_dynamic && filedata->dynamic_strings != NULL && filedata->dynamic_symbols != NULL)
	{
		unsigned long si;
		...
		for (hn = 0; hn < filedata->ngnubuckets; ++hn)
			if (filedata->gnubuckets[hn] != 0)
			{
				...
				for (off = filedata->gnubuckets[hn] - filedata->gnusymidx;
					 ...
					 off < filedata->ngnuchains && (filedata->gnuchains[off] & 1) == 0;
					 ++off)
					++length;
				lengths[hn] = length;
				if (length > maxlength)
...
process_syminfo(Filedata *filedata ATTRIBUTE_UNUSED)
{
	...
	if (filedata->dynamic_syminfo == NULL || !do_dynamic)
	...
	for (i = 0; i < filedata->dynamic_syminfo_nent; ++i)
	{
		...
		case SYMINFO_BT_PARENT:
			...
			break;
		default:
			if (filedata->dynamic_syminfo[i].si_boundto > 0 && filedata->dynamic_syminfo[i].si_boundto < filedata->dynamic_nent && VALID_DYNAMIC_NAME(filedata, filedata->dynamic_section[filedata->dynamic_syminfo[i].si_boundto].d_un.d_val))
			{
...
dump_section_as_ctf(Elf_Internal_Shdr *section, Filedata *filedata)
{
	...
	ctf_arc_symsect_endianness(ctfa, filedata->file_header.e_ident[EI_DATA] != ELFDATA2MSB);
...
initialise_dumps_byname(Filedata *filedata)
{
	...
	for (cur = dump_sects_byname; cur; cur = cur->next)
	{
		...
		for (i = 0; i < filedata->file_header.e_shnum; i++)
			if (SECTION_NAME_VALID(filedata->section_headers + i) && streq(SECTION_NAME(filedata->section_headers + i), cur->name))
			{
	...
	if (options_offset != 0)
	{
		...
		if (eopt)
		{
			...
			while (offset <= sect->sh_size - sizeof(*eopt))
			{
				...
				if (optsize < sizeof(*eopt) || optsize > sect->sh_size - offset)
				{
			...
			while (cnt-- > 0)
			{
				...
				case ODK_REGINFO:
					...
					if (filedata->file_header.e_machine == EM_MIPS)
					{
						...
						/* 32bit form.  */
						if (option.size < (sizeof(Elf_External_Options) + sizeof(Elf32_External_RegInfo)))
						{
							printf(_("<corrupt>\n"));
					...
					else
					{
						...
						if (option.size < (sizeof(Elf_External_Options) + sizeof(Elf64_External_RegInfo)))
						{
							printf(_("<corrupt>\n"));
							error(_("Truncated MIPS REGINFO option\n"));
							cnt = 0;
							   ...
							   const char **pname)
{
	...
	if (filedata->section_headers != NULL && (ba_cache.filedata == NULL || filedata != ba_cache.filedata))
	{
		Elf_Internal_Shdr *symsec;
		...
		for (symsec = filedata->section_headers;
			 ...
			 symsec++)
		{
			if (symsec->sh_type == SHT_SYMTAB && get_symtab(filedata, symsec,
	...
	for (sym = ba_cache.symtab; sym < ba_cache.symtab + ba_cache.nsyms; sym++)
		if (sym->st_value == offset)
		{
			...
			if (ba_cache.strtab[sym->st_name] == '$' && ba_cache.strtab[sym->st_name + 1] != 0 && ba_cache.strtab[sym->st_name + 2] == 0)
				continue;
...
process_archive(Filedata *filedata, bfd_boolean is_thin_archive)
{
	...
	if (do_archive_index)
	{
		...
		else
		{
			...
			for (i = l = 0; i < arch.index_num; i++)
			{
				...
				{
					char *member_name = get_archive_member_name_at(&arch, arch.index_array[i],
																   &nested_arch);

					if (member_name != NULL)
					{
						char *qualified_name = make_qualified_name(&arch, &nested_arch,
																   member_name);
	...
	while (1)
	{
		...
		else if (is_thin_archive)
		{
			...
			filedata->archive_file_offset = arch.nested_member_origin + sizeof arch.arhdr;
